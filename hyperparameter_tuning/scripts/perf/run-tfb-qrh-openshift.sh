...
function err_exit() {
	if [ $? != 0 ]; then
		printf "$*"
		echo
		echo "The run failed. See setup.log for more details"  >> ${LOGFILE}
...
echo "INSTANCES , 50p , 50p_MAXSPIKE , 95p , 95p_MAXSPIKE , 97p , 97p_MAXSPIKE , 99p , 99p_MAXSPIKE , 999p , 999p_MAXSPIKE , 9999p , 9999p_MAXSPIKE , 99999p , 99999p_MAXSPIKE , 50p_HISTO , 95p_HISTO , 97p_HISTO , 99p_HISTO , 999p_HISTO , 9999p_HISTO , 99999p_HISTO" >> ${RESULTS_DIR_ROOT}/Metrics-quantiles-prom.log
echo "INSTANCES , CPU_MAXSPIKE , MEM_MAXSPIKE "  >> ${RESULTS_DIR_ROOT}/Metrics-spikes-prom.log
echo "50p_HISTO , 95p_HISTO , 97p_HISTO , 99p_HISTO , 999p_HISTO , 9999p_HISTO , 99999p_HISTO" >> ${RESULTS_DIR_ROOT}/Metrics-histogram-prom.log
...
do
	...
	# Parse the results
	${SCRIPT_REPO}/perf/parsemetrics-wrk.sh ${TOTAL_ITR} ${RESULTS_SC} ${scale} ${WARMUPS} ${MEASURES} ${NAMESPACE} ${SCRIPT_REPO} >> ${LOGFILE}
	sleep 5
	${SCRIPT_REPO}/perf/parsemetrics-promql.sh ${TOTAL_ITR} ${RESULTS_SC} ${scale} ${WARMUPS} ${MEASURES} ${SCRIPT_REPO} >> ${LOGFILE}
done
...
paste ${RESULTS_DIR_ROOT}/Metrics-prom.log ${RESULTS_DIR_ROOT}/Metrics-wrk.log ${RESULTS_DIR_ROOT}/Metrics-config.log
paste ${RESULTS_DIR_ROOT}/Metrics-quantiles-prom.log ${RESULTS_DIR_ROOT}/Metrics-spikes-prom.log
#cat ${RESULTS_DIR_ROOT}/app-calc-metrics-measure-raw.log
#cat ${RESULTS_DIR_ROOT}/server_requests-metrics-${TYPE}-raw.log
